...
View::View(const Napi::CallbackInfo &info)
    : _asyncCtx(info.Env(), "View::commonCb()"), _isDestroyed(false), _qmlui(nullptr) {
	NAPI_ENV;
...
void View::commonCb(QmlUi *ui, const char *name, const char *json) {
	...
	Napi::Value objVal = _converter.MakeCallback(view->_that.Value(), 1, &argv, view->_asyncCtx);

	eventEmit(view->_that.Value(), name, 1, &objVal, view->_asyncCtx);
}


JS_METHOD(View::_init) {
	NAPI_ENV;
...
JS_IMPLEMENT_METHOD(View, _libs) {
	THIS_CHECK;
	REQ_STR_ARG(0, str);
...
JS_IMPLEMENT_METHOD(View, _resize) {
	THIS_CHECK;
	REQ_INT32_ARG(0, w);
	REQ_INT32_ARG(1, h);
...
JS_IMPLEMENT_METHOD(View, _mouse) {
	THIS_CHECK;
...
JS_IMPLEMENT_METHOD(View, _keyboard) {
	THIS_CHECK;
	REQ_INT32_ARG(0, type);
	REQ_INT32_ARG(1, key);
...
JS_IMPLEMENT_METHOD(View, _load) {
	THIS_CHECK;
...
JS_IMPLEMENT_METHOD(View, _invoke) {
	THIS_CHECK;
	REQ_STR_ARG(0, obj);
...
JS_IMPLEMENT_METHOD(View, _destroy) {
	THIS_CHECK;
...
JS_IMPLEMENT_GETTER(View, isDestroyed) {
	THIS_CHECK;
