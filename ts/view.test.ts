...
const properties = ['width', 'height', 'w', 'h', 'wh', 'size', 'textureId', 'isLoaded'] as const;
...
const texturePromise = Promise.race([
	...
	new Promise<number | null>((res) => {
		view.on('reset', (id: number | null) => res(id));
	}),
]);
...
const tested = describe('Qml View', () => {
	...
	it('has all dynamic methods', () => {
		for (const name of methods) {
			assert.strictEqual(typeof view[name], 'function', `Method "${name}" is missing.`);
		}
	});

	it('has all properties', () => {
		for (const name of properties) {
			assert.notStrictEqual(view[name], undefined, `Property "${name}" is missing.`);
		}
	});
