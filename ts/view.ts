...
export type TMouseEventCommon = TEvent &
	Readonly<{
		buttons: number;
		x: number;
		y: number;
...
export type TMouseEventPress = TMouseEventCommon &
	Readonly<{
		button: number;
	}>;

export type TMouseEventWheel = TMouseEventCommon &
	Readonly<{
		wheelDelta: number;
	}>;
...
export type TOptsView = Readonly<
	Partial<{
...
type TViewBinding = TNativeViewInstance &
	Emitter & {
